...
namespace OppoCraft
{
    class UnitAnimationAdditions
    {
        public static void Render(Unit unit, RenderSystem render)
        {
            ...
            Rectangle bar = new Rectangle(0, 0, render.primRect.Bounds.Width, 3);

            render.spriteBatch.Draw(render.primDot, position, bar, Microsoft.Xna.Framework.Color.Red);

            bar = new Rectangle(0, 0, render.primRect.Bounds.Width*unit.currHP/unit.maxHP, 3);
            render.spriteBatch.Draw(render.primDot, position, bar, Microsoft.Xna.Framework.Color.GreenYellow);


            string text=unit.currHP+"/"+unit.maxHP+"\n";
            foreach (Task task in unit.task.tasks)
            {
                //text += task.GetType().ToString() + "\n";
            }
